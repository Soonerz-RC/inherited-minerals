...
// ---------------------------------------------------------------------------
// Stubbed AI assistant.
//
// v1 returns safe, canned educational answers keyed off simple intent matching.
// It NEVER gives legal/tax advice. To wire up a real LLM later, replace the
// body of `generateAssistantReply` with a call to your LLM provider, keeping the
// same AssistantResponse shape and prepending ASSISTANT_DISCLAIMER. See
// skills/website-building/shared/20-llm-api.md for the integration pattern.
// ---------------------------------------------------------------------------
function generateAssistantReply(raw: string): AssistantResponse {
...
export async function registerRoutes(
  ...
  // --- Private review / sell inquiry ---
  app.post("/api/review-requests", async (req, res) => {
    try {
      const data = insertReviewRequestSchema.parse(req.body);
      const created = await storage.createReviewRequest(data);
  ...
  app.get("/api/review-requests", async (_req, res) => {
    res.json(await storage.listReviewRequests());
  });
  ...
  app.post("/api/questions", async (req, res) => {
    try {
      ...
      const created = await storage.createQuestion(data);
      res.status(201).json(created);
    } catch (err) {
      if (err instanceof ZodError) {
        return res.status(400).json({ message: fromZodError(err).message });
